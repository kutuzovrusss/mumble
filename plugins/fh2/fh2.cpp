...
// Identity ptrs
procptr_t commander_ptr, squad_leader_ptr, squad_state_ptr, team_state_ptr, voip_ptr, voip_com_ptr, target_squad_ptr, ipport_ptr;
...
inline bool resolve_ptrs() {
	...
	Context:
		IP:Port of server:  ForgottenHope2.exe+0x0059E348                                  char[128] ip:port of the server
	...
	*/
	...
	procptr_t base_ipport_ptr = peekProcPtr(pmodule_bf2 + 0x0059E348);
	if (!base_ipport_ptr) return false;

	ipport_ptr = peekProcPtr(base_ipport_ptr + 0x108);
...
static int trylock(const std::multimap<std::wstring, unsigned long long int> &pids) {
	if (! initialize(pids, L"ForgottenHope2.exe", L"BF2Audio.dll"))
		return false;

	pmodule_bf2		= getModuleAddr(L"ForgottenHope2.exe");
...
static const std::wstring longdesc() {
	return std::wstring(L"Supports Forgotten Hope 2 v >= 2.57");
}

static std::wstring description(L"Forgotten Hope 2 v2.57");
static std::wstring shortname(L"Forgotten Hope 2");
